...
class AgentClient {
  ...
  async getTodayFile(): Promise<{ content: string; message?: string }> {
    const response = await fetch(`http://localhost:8000/today_file`);
    
    if (!response.ok) {
      throw new Error(`Failed to get today file: ${response.statusText}`);
    }

    const data = await response.json();
    
    // Handle the different response format from port 8000
    if (data.status === 'success') {
      return { content: data.content };
    } else {
      return { content: '', message: data.message || 'No data available' };
    }
  }
